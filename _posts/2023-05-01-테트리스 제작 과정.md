...
##### 텍스트 출력 부분에서 메모리 누수가발생함
프로그램을 동작시키면 1초마다 300mb씩 메모리사용량이 증가하다 컴퓨터가 멈추는 치멍적인 버그가 발생했다.

SDL_RenderCopy함수로 데이터가 카피된이후 할당한 메모리를 해제하여 문제를 해결하였다.
...
키보드 입력을받고 그 입력에 맞춘 블록의 움직임을 구현해야한다.

키보드 입력은 SDL함수를 통해 간단히 구현할 수 있다. 
...
Position CurrentBlockPosition[4];

...

	int index=0;
	for(int i=0;i<4;i++){//current block position
		for(int j=0;j<2;j++){
			matrix[2+j][14+i] = blocks[currentBlock][j][i];
			CurrentBlockPosition[index].x=14+i;
			CurrentBlockPosition[index].y=2+j;
			index++;
		}
	}
...
충돌감지 함수작성,현재 컨트롤할 블록의 위치 정보를 알고있으니

matrix함수에서 움직이려고 하는 블록이 0아아닌 다른 값이 존재한다면 충돌로 인지하고(이떄 본인 블럭의 값이라면 무시함)
...
```c
void move_block(SDL_Event e){
	bool b_moveable = true;
	switch (e.key.keysym.sym) {
                case SDLK_SPACE:
                    break;
                case SDLK_LEFT:
                ...
                case SDLK_DOWN:
		    for(int i =0;i<4;i++){
			    if(matrix[CurrentBlockPosition[i].y+1][CurrentBlockPosition[i].x] >0 && matrix[CurrentBlockPosition[i].y+1][CurrentBlockPosition[i].x] != currentBlock){
		    ...
		    if (b_moveable){
			    for(int i=0;i<4;i++){
				    matrix[CurrentBlockPosition[i].y][CurrentBlockPosition[i].x] = EMPTY;
			    }
			    for(int i=0;i<4;i++){
				    matrix[CurrentBlockPosition[i].y+1][CurrentBlockPosition[i].x] = currentBlock;
				    CurrentBlockPosition[i].y+=1;
			    }
...
#### 스페이스바 함수
스페이스바를 누르면 블록이 떨어질수 있는 위치까지 바로 떨어진다.
이부분은 down키를 20번 반복한 효과를줘서 구현했다.
...
#### 바닥감지
블록이 바닥에 닿으면 다음 블록이 현재블록으로 떨어지고 다음블록을 랜덤하게 뽑아야한다.

무브블록함수에서 블록이아래로 떨어질떄 어느한 블록이라도 바닥에 닿으면

flag가 켜지고 

flag가 켜지면 이벤트가발생해서

새로운 블록을 스폰하고 flag를 비활성화하는 식으로 구현해야겠다.
```c
if(CurrentBlockPosition[i].y+1 == 21 || (matrix[CurrentBlockPosition[i].y+1][CurrentBlockPosition[i].x] >0 && (matrix[CurrentBlockPosition[i].y+1][CurrentBlockPosition[i].x] != currentBlock))){
						b_touchBottom = true;
					} 
```

##### 문제점
이전에 깔렷던 블록과 현재블록모양이 같은경우 이전블록 위치에 덮어쓰기가 된다.

현재 테트리스블록의 가장 하단블록 +1 위치에 0이상의 값이 존재한다면 닿았다고 구현해야겠다.

최하단블록을 하나만 감지하면 빈공간 사이에 블록이 떨어질떄 flag로 진입하질않는다.

또한 모든블록의 y+1방향을 검사하면 현재 컨트롤하고있는 블록도 검사하게되어

스폰되자마자 충돌감지을 감지해버린다. 

한블록을 검사할때 현재 블록의 모든부분을 검사하는 비효율적인 구현으로 일단 넘어가야겠다.

아니면 해쉬기능으로 구현할 수 있을거같다. 
